...
begin
    ...
    try
        try
            ...
            on e: Exception do
            begin
                GravaLog(TDModulo(f_DModulo).V_ArquivoLog, 0, '', 'modulo', '111220230848', 'Erro de excecao no ThProcesso.Execute ' +
...
begin
    try
        ...
        begin
            Result := 54;
            GravaLog(TDModulo(VO_Modulo).V_ArquivoLog, 0, '', 'modulo', '081220231707', 'Erro de excecao no inicializar ' +
                e.ClassName + '/' + e.Message, '', 54, 1);
        end;
    end;
end;
    ...
    try
        try
            ...
            if ((TDModulo(VP_Modulo).V_DComunicador.V_ConexaoCliente.ServidorHost <> VP_Host) or
                ...
                (TDModulo(VP_Modulo).V_DComunicador.V_ConexaoCliente.Identificacao <> VP_Identificacao) or
                (TDModulo(VP_Modulo).V_DComunicador.V_ConexaoCliente.Chave_Comunicacao <> VP_ChaveComunicacao)) then
            begin
...
begin
    ...
    try
        ...
        begin
            Result := 1;
            GravaLog(TDModulo(VP_Modulo).V_ArquivoLog, 0, '', 'modulo', '111220230845', 'Erro de excecao no solicitacao ' +
                e.ClassName + '/' + e.Message, '', 1, 1);
        end;
    end;
end;
...
begin
    ...
    try
        try
            ...
            begin
                Result := 1;
                GravaLog(TDModulo(VP_Modulo).V_ArquivoLog, 0, '', 'modulo', '111220230846', 'Erro de excecao no solicitacaoblocante ' +
                    e.ClassName + '/' + e.Message, '', 1, 1);
            end;
        end;
    finally
...
begin
    try
        ...
        begin
            Result := 1;
            GravaLog(TDModulo(VP_Modulo).V_ArquivoLog, 0, '', 'modulo', '111220230847', 'Erro de excecao no modulostatus ' +
                e.ClassName + '/' + e.Message, '', 1, 1);
        end;
    end;
end;

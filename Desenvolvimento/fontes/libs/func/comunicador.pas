...
type
  ...
  public
    ...
    V_ServidorRecebimento: TServidorRecebimento;
    //procedure disparado no envento que o IdTCPServidor recebe passa a conexão do cliente
    V_ClienteRecebimento: TRetorno;
...
begin
  ...
  try
    try
      ...
      if ((VL_Mensagem.Comando = '0021') and (VL_Mensagem.ComandoDados = 'R')) then
        // troca da chave rsa
      begin
        if Assigned(TDComunicador(f_DComunicador).V_ConexaoCliente) then
        begin
          TDComunicador(f_DComunicador).V_ConexaoCliente.setModuloPublico(
            VL_Mensagem.GetTagAsAstring('0008'));
          TDComunicador(f_DComunicador).V_ConexaoCliente.setExpoentePublico(
          ...
          VL_Mensagem.Limpar;
          VL_Mensagem.AddComando('0103', 'S'); // SOLICITA DESAFIO DE CHAVE
          VL_Mensagem.AddTag('00E3',
            TDComunicador(f_DComunicador).V_ConexaoCliente.Rsa.EncryptString(VL_Dados));
      ...
      if ((VL_Mensagem.Comando = '0103') and (VL_Mensagem.ComandoDados = 'R')) then
        // retorno do desafio de chave
      begin
        if VL_Mensagem.GetTagAsAstring('00E3') = '' then
          // transacao criptografada
        begin
          VL_Mensagem.Limpar;
          VL_Mensagem.AddComando('0026', '92');
      ...
      begin
        ...
        if Assigned(TDComunicador(
          ...
          TDComunicador(f_DComunicador).V_ClienteRecebimentoModulo(
            ...
            PChar(VL_Dados), TDComunicador(f_DComunicador).V_Modulo);

      end
      else
        TDComunicador(f_DComunicador).V_EventoSocketAguardaResposta.parar(
          VL_TransmissaoID);
...
begin
  ...
  try
    begin
      ...
      TDComunicador(VP_DComunicador).idTCPCliente.Host :=
        TDComunicador(VP_DComunicador).V_ConexaoCliente.ServidorHost;
      TDComunicador(VP_DComunicador).idTCPCliente.Port :=
        TDComunicador(VP_DComunicador).V_ConexaoCliente.ServidorPorta;
      ...
      try
        ...
        on e: Exception do
        begin
          GravaLog(TDComunicador(VP_DComunicador).V_ArquivoLog,
            0, '', 'ConectarCliente', '281120231227',
      ...
      begin
        Result := 26;
        GravaLog(TDComunicador(VP_DComunicador).V_ArquivoLog,
          0, '', 'ConectarCliente', '081220231450', 'erro ao conectar', '', Result, 1);
        Exit;
      end;


      VL_Mensagem.Limpar;
      VL_Mensagem.AddComando('0021', 'S');
      // PEDIDO DE CONEXÃO(troca de chave rsa)
      VL_Mensagem.AddTag('0008',
        TDComunicador(VP_DComunicador).V_ConexaoCliente.ModuloPublico); // CHAVE PUBLICA MODULO
      VL_Mensagem.AddTag('0027',
        TDComunicador(VP_DComunicador).V_ConexaoCliente.ExpoentePublico);
      ...
      begin
        GravaLog(TDComunicador(VP_DComunicador).V_ArquivoLog,
          0, '', 'ConectarCliente', '081220231451',
      ...
      begin
        GravaLog(TDComunicador(VP_DComunicador).V_ArquivoLog,
          0, '', 'ConectarCliente', '081220231452',
...
begin
  try
    ...
    begin
      Result := 83;
      GravaLog(TDComunicador(VP_DComunicador).V_ArquivoLog, 0,
        '', 'TDComunicador.ClienteVerificaConexao', '160920221056',
        ' ' + e.ClassName + '/' + e.Message, '', 1, 1);
      DesconectarCliente(VP_DComunicador);
    end;
  end;
...
begin
  ...
  TDComunicador(Sender).V_ClassePai := nil;
...
begin
  try
    GravaLog(TDComunicador(self).V_ArquivoLog, 0, 'ModuloDescarrega',
      'opentefnucleo', '141120231638',
    ...
    on e: Exception do
      GravaLog(TDComunicador(self).V_ArquivoLog, 0, '',
        'TDComunicador.DataModuleDestroy', '111220231640',
        ' ' + e.ClassName + '/' + e.Message, '', 1, 1);
  end;

  GravaLog(TDComunicador(self).V_ArquivoLog, 0, 'ModuloDescarrega',
    'opentefnucleo', '141120231639', 'fim do TDComunicador.DataModuleDestroy', '', 0, 3);
end;
...
begin
  ...
  try
    try
      ...
      GravaLog(TDComunicador(Self).V_ArquivoLog, 0, '',
        'comunicador', '141120231415', 'TThServidorProcessaRecebe.Execute',
        VL_DadosRecebidos, 0, 4);
      ...
      if ((VL_Mensagem.Comando = '0021') and (VL_Mensagem.ComandoDados = 'S')) then
        // solicita conexao(troca de chave rsa)
      begin
        V_ConexaoCliente.setModuloPublico(VL_Mensagem.GetTagAsAstring('0008'));
        // CHAVE PUBLICA MODULO
        V_ConexaoCliente.setExpoentePublico(VL_Mensagem.GetTagAsAstring('0027'));
        // CHAVE PUBLICA EXPOENTE

        VL_Mensagem.Limpar;
        VL_Mensagem.AddComando('0021', 'R'); // troca de chave rsa aceita
        VL_Mensagem.AddTag('0008', VL_TConexao.ModuloPublico);
        // CHAVE PUBLICA MODULO
        VL_Mensagem.AddTag('0027', VL_TConexao.ExpoentePublico);
        // CHAVE PUBLICA EXPOENTE
      ...
      if ((VL_Mensagem.Comando = '0103') and (VL_Mensagem.ComandoDados = 'S')) then
        // solicita desafio de chave
      begin
        if VL_Mensagem.GetTagAsAstring('00E3') = '' then
          // transacao criptografada
        begin
        ...
        begin
          ...
          VL_TConexao.Aes.GenerateKey(VL_Mensagem.GetTagAsAstring('0023'));
        ...
        VL_Mensagem.Limpar;
        VL_Mensagem.AddComando('0103', 'R'); // RETORNO DO DESAFIO DE CHAVE
        VL_Mensagem.AddTag('00E3', V_ConexaoCliente.Rsa.EncryptString(VL_Dados));
        // transacao criptografada
...
begin
  try
    ...
    on e: Exception do
    begin
      GravaLog(V_ArquivoLog, 0, '', 'comunicador', '081220231619',
        'IdTCPClienteConnected' + e.ClassName + '/' + e.Message, '', 1, 1);
    end;
  end;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        VL_Temporizador :=
          TTemporizador.Create(TDComunicador(VP_DComunicador).V_ListaThreads);
        VL_Temporizador.V_Executado := False;
        VL_Temporizador.V_ID := VP_Transmissao_ID;

        VP_Mensagem.AddComando('00D1', VL_Temporizador.V_ID);

        TDComunicador(VP_DComunicador).V_EventoSocketAguardaResposta.add(
          VL_Temporizador);
        VL_AContext.Connection.Socket.WriteLn(
          base64.EncodeStringBase64(VP_Mensagem.TagsAsString));
        ...
        begin
          TDComunicador(VP_DComunicador).V_EventoSocketAguardaResposta.remove(
            VL_Temporizador);
          VL_Temporizador.Free;
          Exit;
        end;
        ...
        if Assigned(VP_Procedimento) then
          VP_Procedimento(Result, VP_Transmissao_ID,
            VL_Temporizador.V_Dados, TTConexao(VL_AContext.Data).ID,
    ...
    begin
      Result := 1;
      GravaLog(V_ArquivoLog, 0, '', 'comunicador', '090620221846',
        'ServidorTransmiteSolicitacaoID' + e.ClassName + '/' + e.Message, '', 1, 1);
    end;
    ...
    begin
      try
        ...
        begin
          ...
          TDComunicador(
            f_DComunicador).IdTCPCliente.IOHandler.CheckForDisconnect;
        ...
        begin
          ...
          begin
            ...
            try
              VL_Dados :=
                TDComunicador(f_DComunicador).IdTCPCliente.Socket.ReadLn;
  ...
  try
    try
      ...
      begin
        ...
        TDComunicador(VP_DComunicador).IdTCPCliente.Socket.WriteLn(
          base64.EncodeStringBase64(VL_Mensagem.TagsAsString));
...
begin
  ...
  try
    try
      ...
      if Result <> 0 then
      begin
        GravaLog(V_ArquivoLog, 0, '', 'comunicador', '190520222159',
          'Erro na TDComunicador.ClienteTransmiteSolicitacao ', '', Result, 1);
        Exit;
      end;
      ...
      if Result <> 0 then
      begin
        GravaLog(V_ArquivoLog, 0, '', 'comunicador', '120920220834',
          'Erro na TDComunicador.ClienteTransmiteSolicitacao ', '', Result, 1);
        Exit;
      end;
      ...
      begin
        ...
        begin
          Result := 24;
          TDComunicador(VP_DComunicador).V_ConexaoCliente.Status :=
            csDesconectado;
          if TDComunicador(VP_DComunicador).idTCPCliente.Connected then
            DesconectarCliente(VP_DComunicador);
          GravaLog(V_ArquivoLog, 0, '', 'comunicador',
            '210520222014', 'Erro na TDComunicador.ClienteTransmiteSolicitacao ', '', Result, 1);
          Exit;
        ...
        if Assigned(VP_Procedimento) then
          VP_Procedimento(TDComunicador(VP_DComunicador).V_ClassePai, PChar(VO_Transmissao_ID), V_ProcID,
            Result, PChar(VL_Dados));
      end;
    except
      on e: Exception do
      begin

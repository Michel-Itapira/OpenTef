...
type
  ...
  TDComunicadorVBI = class(TDataModule)
...
procedure TDComunicadorVBI.DataModuleCreate(Sender: TObject);
begin
  fSequencia := 1;
...
procedure TDComunicadorVBI.concentrar;
begin
  if IdTCPClient.Connected then
    Exit;

  IdTCPClient.Host := fHost;
  IdTCPClient.Port := fPorta;
  IdTCPClient.Connect;
  IdTCPClient.IOHandler.MaxLineLength := MaxInt;
  IdTCPClient.IOHandler.SendBufferSize := MaxInt;
end;

procedure TDComunicadorVBI.antesEnviarMensagem(var VO_DadosEnviados: TTagVBI);
begin
...
function TDComunicadorVBI.enviarMensagemSocket(VP_DadosEnviados: string): string;
begin
   Result := '';

  IdTCPClient.Socket.Write(VP_DadosEnviados);
  IdTCPClient.Socket.Write(C_Comunicacao_Terminacao);

  Result := IdTCPClient.Socket.ReadLn(C_Comunicacao_Terminacao);
end;

function TDComunicadorVBI.verificaErroSequecial(var VO_DadosEnviados: TTagVBI; var VO_DadosRecebidos: TTagVBI): integer;
var
  VL_Dados: string;
begin
  Result := 0;
...
function TDComunicadorVBI.verificaTagErro(VP_DadosEnviados: TTagVBI): TVBIRetorno;
begin
...
procedure TDComunicadorVBI.setConfig(VP_Host: string; VP_Porta: integer);
begin
...
function TDComunicadorVBI.enviarMensagem(VP_DadosEnviados: TTagVBI; var VO_DadosRecebidos: TTagVBI): TVBIRetorno;
var
  VL_Dados: string;
begin
  limparRetornoVBI(Result);

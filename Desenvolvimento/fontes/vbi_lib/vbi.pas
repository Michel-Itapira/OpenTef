...
type
  ...
  TVBI = class
  private
    function iniciarComunicacao(VP_Comunicador: TDComunicadorVBI): TVBIRetorno;
  ...
  public
    ...
    function consultarMenu(VP_Comunicador: TDComunicadorVBI; var VO_Menu: TVBIMenuArray): TVBIRetorno;
    function vendaCartao(VP_Comunicador: TDComunicadorVBI; VP_Valor: real; VP_TransacaoTipo: string; VP_QuantidadeParcela: integer): TVBIRetorno;
  end;
...
function TVBI.consultarMenu(VP_Comunicador: TDComunicadorVBI; var VO_Menu: TVBIMenuArray): TVBIRetorno;
var
...
function TVBI.vendaCartao(VP_Comunicador: TDComunicadorVBI; VP_Valor: real; VP_TransacaoTipo: string; VP_QuantidadeParcela: integer): TVBIRetorno;
var
  VL_DadosEnviados, VL_DadosRecebidos: TTagVBI;
  VL_Menu: TStringArray;
begin
  ...
  try
    ...
    VL_DadosEnviados.limpar;
    VL_DadosEnviados.addTag('servico', 'executar');
    VL_DadosEnviados.addTag('retorno', '1');
    VL_DadosEnviados.addTag('transacao', 'Cartao Vender');

    if VP_TransacaoTipo = 'C' then
    begin
      if VP_QuantidadeParcela > 1 then
      begin
        VL_DadosEnviados.addTag('transacao_pagamento', 'Parcelado');
        VL_DadosEnviados.addTag('transacao_parcela', IntToStr(VP_QuantidadeParcela));
        VL_DadosEnviados.addTag('transacao_financiado', 'Estabelecimento');
      end
      else
        VL_DadosEnviados.addTag('transacao_pagamento', 'A vista');

      VL_DadosEnviados.addTag('transacao_tipo_cartao', 'Credito');
    end
    else
    begin
      VL_DadosEnviados.addTag('transacao_tipo_cartao', 'Debito');
      VL_DadosEnviados.addTag('transacao_pagamento', 'A vista');
    end;

    VL_DadosEnviados.addTag('transacao_valor', FloatToStr(VP_Valor));
...
function TVBI.iniciarComunicacao(VP_Comunicador: TDComunicadorVBI): TVBIRetorno;
var
  VL_DadosEnviados, VL_DadosRecebidos: TTagVBI;
begin
  limparRetornoVBI(Result);

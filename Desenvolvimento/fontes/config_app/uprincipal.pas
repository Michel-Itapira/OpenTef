...
type
  ...
  private
    ...
    procedure AlterarRegistro(VP_TagTabela: string; VP_Tabela: TRxMemoryData; VP_Tag: string; VP_ID: int64; VP_TagComando, VP_TagComandoDados: string; VP_TabelaCarregamento: string);
  ...
  TTefDesconectar = function(VP_Com: pointer): integer; cdecl;
  TTLogin = function(VP_Com: pointer; VP_Host: PChar; VP_Porta: integer; VP_Chave: PChar; VP_Versao_Comunicacao: integer; VP_Senha: PChar; VP_Tipo: PChar;
...
begin
  ...
  try
    ...
    begin
      ...
      VL_Conf.WriteString('Servidor', 'Identificacao', '');
      VL_Conf.Free;
      VL_Conf := nil;
    end;
    ...
    begin
      ...
      F_Identificacao := VL_Conf.ReadString('Servidor', 'Identificacao', '');
    end;
    ...
    VL_Codigo := F_Login(F_Com, PChar(F_Host), F_Porta, PChar(F_Chave), C_Versao_Mensagem, PChar(ESenha.Text), PChar(VL_Tipo), F_Terminal_ID, PChar(F_Identificacao));

    if VL_Codigo <> 0 then
    begin
      mensagemerro(VL_Codigo, VL_Retorno);

      VL_DescricaoErro := VL_Retorno;
      F_MensagemDispose(VL_Retorno);

      ShowMessage('Erro:' + IntToStr(VL_Codigo) + #13 + VL_DescricaoErro);
      Exit;
    end;
...
begin
  ...
  try
    ...
    //carrega opentef.ini
    VL_Conf := TIniFile.Create(PChar(ExtractFilePath(ParamStr(0)) + '..\..\opentef\win64\open_tef.ini'));
    if VL_Conf.ReadInteger('Servidor', 'Porta', 0) <> 0 then
    begin
      VL_FConfg.TabOpenTefEPorta.Text := IntToStr(VL_Conf.ReadInteger('Servidor', 'Porta', 0));
      VL_FConfg.TabOpenTefCPortaAtiva.Checked := VL_Conf.ReadBool('Servidor', 'Ativa', False);
    end
    else
    begin
    ...
    if VL_Conf.ReadInteger('Servidor', 'Porta', 0) <> 0 then
    begin
      VL_FConfg.TabConfiguradorEPorta.Text := IntToStr(VL_Conf.ReadInteger('Servidor', 'Porta', 0));
      VL_FConfg.TabConfiguradorEHost.Text := VL_Conf.ReadString('Servidor', 'Host', '');
      VL_FConfg.TabConfiguradorEChave.Text := VL_Conf.ReadString('Servidor', 'Chave', '');
    end
    else
    begin
      ...
      case VL_Retorno of
        ...
        begin
          ...
          begin
            ...
            ShowMessage('ERRO:' + VL_Tag + VL_RDescricaoErro);
            if MDAdquirente.Locate('ID', 0, []) then
              MDAdquirente.Delete;
            Exit;
          end;
...
begin
  ...
  try
    ...
    case VL_Retorno of
      '0026':
      begin
        VL_Mensagem.GetTag('0026', VL_Codigo);
        mensagemerro(VL_Codigo, VL_RDescricaoErro);
      ...
      begin
        ...
        if vl_tag <> '0' then
        begin
          VL_Mensagem.GetTag('004D', VL_Codigo);
          mensagemerro(VL_Codigo, VL_RDescricaoErro);
...
begin
  ...
  try
    ...
    if (MDLojaFuncao.Active = False) then
      exit;
    F_Navegar := False;
    if ((TabLojaFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabLojaFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  try
    ...
    if (MDLojaModuloConfFuncao.Active = False) then
      exit;
    F_Navegar := False;
    if ((TabLojaTabFuncaoDadosGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabLojaTabFuncaoDadosGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  if ((MDMultiLoja.Active) and (MDLoja.Active) and (MDMultiLojaLoja.Active) and ((VP_TabelaCarregamento = 'MULTILOJA') or (VP_TabelaCarregamento = 'TODAS'))) then
  begin
  ...
  begin
    ...
    TabPdvEChave.Text := MDPdv.FieldByName('CHAVE_COMUNICACAO').AsString;
    TabPdvCkHabilitar.Checked := MDPdv.FieldByName('HABILITADO').AsString = 'T';
  ...
  begin
    ...
    TabConfEChave.Lines.Text := MDConfigurador.FieldByName('CHAVE_COMUNICACAO').AsString;
  ...
  begin
    ...
    TabConfiguracaoEChave.Text := MDModuloConfig.FieldByName('CHAVE_COMUNICACAO').AsString;
...
begin
  ...
  try
    ...
    begin
      ...
      MDPdv.FieldByName('CHAVE_COMUNICACAO').AsString := TabPdvEChave.Text;
    ...
    begin
      ...
      MDConfigurador.FieldByName('CHAVE_COMUNICACAO').AsString := TabConfEChave.Lines.Text;
...
begin
  ...
  try
    ...
    Result := F_SolicitacaoBlocante(F_Com, VL_PTag, VL_PRetorno, C_TempoSolicitacao);
...
procedure Tfprincipal.AlterarRegistro(VP_TagTabela: string; VP_Tabela: TRxMemoryData; VP_Tag: string; VP_ID: int64; VP_TagComando, VP_TagComandoDados: string; VP_TabelaCarregamento: string);
var
  VL_Mensagem: TMensagem;
  VL_Tabela: string;
  VL_Tag: ansistring;
  ...
  try
    ...
    case VL_Retorno of
      ...
      begin
        ...
        //TABELA LOJA
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '003E')) then
        begin
        ...
        //TABELA MULT-LOJA
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0080')) then
        begin
        ...
        //TABELA TAG
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0081')) then
        begin
        ...
        //ADQUIRENTE
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0082')) then
        begin
        ...
        //BIN
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0083')) then
        begin
        ...
        //PINPAD
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '008D')) then
        begin
        ...
        //PDV
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '008E')) then
        begin
        ...
        //configurador
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '008F')) then
        begin
        ...
        //MODULO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0090')) then
        begin
        ...
        //PESQUISA_MODULO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0095')) then
        begin
        ...
        //MODULO_CONFIG
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '003A')) then
        begin
        ...
        //MODULO_FUNC
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0092')) then
        begin
        ...
        //MODULO_CONF_FUNCAO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0093')) then
        begin
        ...
        //MULTLOJA_MODULO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0094')) then
        begin
        ...
        //MULTLOJA_FUNCAO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '009C')) then
        begin
        ...
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00D0')) then
        begin
        ...
        //LOJA_MODULO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00A7')) then
        begin
        ...
        //LOJA_FUNCAO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00A9')) then
        begin
        ...
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '0098')) then
        begin
        ...
        //PINPAD_FUNCAO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00B4')) then
        begin
        ...
        //PDV_FUNCAO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00C1')) then
        begin
        ...
        //PDV_MODULO
        if ((VP_CarregaTodasTabelas) and (VP_TagTabelaEspecifica = '')) or ((VP_CarregaTodasTabelas = False) and (VP_TagTabelaEspecifica = '00C7')) then
        begin
...
begin
  ...
  if F_ComLib = 0 then
  begin
    ShowMessage('Não foi possível carregar a biblioteca');
...
begin
  ...
  try
    ...
    begin
      ...
      MDLoja.Locate('ID', MDPdv.FieldByName('LOJA_ID').AsInteger, []);
      VL_ValorChave := MDLoja.FieldByName('DOC').AsString;
      VL_ValorChave := VL_ValorChave + MDPdv.FieldByName('ID').AsString;
      MDLojaModuloConf.DisableControls;
      MDLojaModuloConf.Locate('LOJA_ID', MDPdv.FieldByName('LOJA_ID').AsInteger, []);
      VL_ValorChave := VL_ValorChave + MDLojaModuloConf.FieldByName('CODIGO').AsString;
      MDLojaModuloConf.EnableControls;
...
begin
  ...
  try
    ...
    if ((TabModuloTabDadosFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabModuloTabDadosFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  try
    ...
    if ((MDMultiLojaModuloConfFuncao.Active = False) or (MDMultiLojaModuloConfFuncao.RecordCount < 1)) then
      exit;
    if ((TabMultLojaConfFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabMultLojaConfFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  try
    ...
    if ((MDMultiLojaFuncao.Active = False) or (MDMultiLojaFuncao.RecordCount < 1)) then
      exit;
    F_Navegar := False;
    if ((TabMultLojaFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabMultLojaFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  try
    ...
    if (MDPinPadFuncao.Active = False) then
      exit;
    F_Navegar := False;
    if ((TabPinPadFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabPinPadFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin

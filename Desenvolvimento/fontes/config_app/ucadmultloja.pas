...
begin
  ...
  try
    ...
    if EID.Text = '' then
    begin
      ShowMessage('Nenhuma Loja Ativa foi pesquisada para a inclusão');
      Exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      MDLojaNG.Filter := 'MULTILOJA_ID <= 0 AND MULT=''F''';
    ...
    begin
      MDLojaG.Filter := 'MULTILOJA_ID=' + EID.Text;
...
begin
  ...
  try
    ...
    begin
      ShowMessage('NÃO EXISTE MULTI-LOJA CADASTRADA PARA MONTAR GRUPO');
      Exit;
    end;
...
begin
  ...
  try
    ...
    if ((MDMultiLojaModuloConfFuncao.Active = False) or (MDMultiLojaModuloConfFuncao.RecordCount < 1)) then
      exit;
    if ((TabModuloFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabModuloFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
...
begin
  ...
  try
    ...
    if ((TabMultiLojaFuncaoGrid.SelectedColumn.FieldName = 'VALIDADO_F') or (TabMultiLojaFuncaoGrid.SelectedColumn.FieldName = 'HABILITADO_F')) then
    begin
    ...
    begin
      ...
      if Components[i] is TRxMemoryData then
        if TRxMemoryData(Components[i]).Active then
          if ((TRxMemoryData(Components[i]) <> TRxMemoryData(MDPesquisaModulo)) and (TRxMemoryData(Components[i]) <> TRxMemoryData(MDLoja))) then

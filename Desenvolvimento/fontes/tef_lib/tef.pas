...
uses
    Classes, SysUtils, comunicador, funcoes, def, LbRSA, LbAsym, LbClass;
...
function login(VP_Host: PChar; VP_Porta, VP_ID: integer; VP_ChaveTerminal: PChar; VP_Versao_Comunicacao: integer): integer; cdecl;
...
begin
    try
     ...
     2º recebe tag 00E3 então operadora quer que criptografe dados
        ...
        VL_TagDados := '';
        if VL_Mensagem.Comando() = '0111' then  // solicita chave publica
        begin
            VL_Mensagem.AddComando('0111', 'R'); // retorno da chave publica
            VL_Mensagem.AddTag('0008', DTef.CriptoRsa.PublicKey.ModulusAsString); //modulos
            VL_Mensagem.AddTag('0027', DTef.CriptoRsa.PublicKey.ExponentAsString);  //expoente
            VL_Mensagem.AddTag('00E4', Ord(DTef.CriptoRsa.PublicKey.KeySize));   //tamanho chave
            ...
            if VL_Erro <> 0 then
                GravaLog(F_ArquivoLog, 0, '', 'tef', '310820221757', 'Erro na SolicitaParaCliente ', '', VL_Erro);
            Exit;
        end;
        ...
        begin
            ...
            GravaLog(F_ArquivoLog, 0, '', 'tef', '210920221056', 'SolicitaParaCliente recebeu comando com erro', '', StrToInt(VL_String));
            Exit;
        end;
        ...
        VL_ChaveTamanho := VL_Mensagem.GetTagAsInteger('00E4');   //tamanho chave
        VL_ChaveExpoente := VL_Mensagem.GetTagAsAstring('0027');  //expoente
        VL_ChaveModulo := VL_Mensagem.GetTagAsAstring('0008');  //modulos
        ...
        begin
            ...
            begin
                ...
                begin
                    GravaLog(F_ArquivoLog, 0, '', 'tef', '230820221700', 'Erro na SolicitaParaCliente ', '', 84);
                    Exit;
        ...
        begin
            ...
            if VL_Erro <> 0 then
            begin
                GravaLog(F_ArquivoLog, 0, '', 'tef', '230820221701', 'Erro na SolicitaParaCliente ', '', VL_Erro);
                Exit;
            end;
        end;
        ...
        begin
            ...
            //   if VL_Mensagem.Comando() = '0111' then
            ...
            begin
                ...
                begin
                    VL_Comando := VL_Mensagem.Comando();
                    VL_String := VL_Rsa.EncryptString(VL_Mensagem.TagsAsString);
                    VL_Mensagem.Limpar;
                    VL_Mensagem.AddTag('00E3', VL_String);
                    VL_Mensagem.AddComando(VL_Comando, 'R');
                    VL_TransacaoDadosPublicos.AddTag('00F1', VL_Chave_00F1); //  chave da transacao
                ...
                if VL_Erro <> 0 then
                begin
                    GravaLog(F_ArquivoLog, 0, '', 'tef', '230820221732', 'Erro na SolicitaParaCliente ', '', VL_Erro);
                    Exit;
                end;
    ...
    try
     ...
     2º recebe tag 00E3 então operadora quer que criptografe dados
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ftransacao.erro := 84;
                        ftransacao.STATUS := tsComErro;
                        GravaLog(F_ArquivoLog, 0, '', 'tef', '240820220911', 'Erro na ThTransacao.execute', '', 84);
                        Exit;
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ftransacao.erro := VL_Erro;
                        ftransacao.STATUS := tsComErro;
                        GravaLog(F_ArquivoLog, 0, '', 'tef', '240820220837', 'Erro na ThTransacao.execute', '', VL_Erro);
                        Exit;
                    end;
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                ftransacao.fMensagem.AddTag('00F1', VL_Chave.TagsAsString);
                            end;
                        end
                        else
                            ftransacao.fMensagem.AddTag(VL_Tag, VL_TagDados);
            ...
            if VL_Criptografa then
            begin
                VL_String := VL_Rsa.EncryptString(ftransacao.fMensagem.TagsAsString);
                VL_Mensagem.AddTag('00E3', VL_String);
...
begin
    ...
    if Assigned(VP_RetornoCliente) then
    ...
    //F_DComunicador.V_ThRecebeEscuta := TThRecebe.Create(F_DComunicador);
...
var
    ...
    VL_Transmissao_ID, VL_Desafio, VL_Desafio_ID: string;
begin
    VL_S := '';
    VL_Dados := '';
    VL_Desafio := '';
    ...
    try
        ...
        if Result <> 0 then
        begin
            GravaLog(F_ArquivoLog, 0, '', 'tef_lib', '190920220851', 'Erro no login', '', Result);
            Exit;
        end;
        ...
        begin
            DTef.CriptoAes.GenerateKey(VP_ChaveTerminal);
            VL_Desafio_ID := FloatToStr(Random(10000)) + '      ';
            VL_Desafio := DTef.CriptoAes.EncryptString(VL_Desafio_ID + DateTimeToStr(Now) + FloatToStr(Random(1000)));
            VL_Mensagem.Limpar;
            ...
            if Result <> 0 then
            begin
                GravaLog(F_ArquivoLog, 0, '', 'tef_lib', '190920220849', 'Erro no login', '', Result);
                Exit;
            end;
            ...
            if VL_S = '0028' then
            begin
                VL_Mensagem.GetTag('0106', VL_Dados);
                if trim(VL_Dados) = trim(VL_Desafio_ID) then
                begin
                    F_DComunicador.V_ConexaoCliente.Status := csLogado;
                    F_DComunicador.V_ConexaoCliente.ConexaoAtivadada := True;
                end
    ...
    try
        try
            ...
            begin
                Result := F_DComunicador.ClienteTransmiteSolicitacao(F_DComunicador, VL_Transmissao_ID, VL_Mensagem, VL_Mensagem, nil, 10000, True);
                if Result <> 0 then
                    GravaLog(F_ArquivoLog, 0, '', 'tef_lib', '190920220854', 'Erro na transacaocreate', '', Result);
            end
            else
            begin
                for VL_I := 1 to VL_Mensagem.TagCount do

...
type
  ...
  TMensagem = class
   ...
   function AddComando(tag,dados:string):integer;
...
begin
  try
   ...
   begin
     ...
     fTags[Length(fTags)-1].Tamanho:=StrToInt64Def(Copy(dados,6, Hex2Dec(fTags[Length(fTags)-1].Qt)),0);
...
function TMensagem.AddComando(tag,dados:string):integer;
var
   i : Integer;
begin
 ...
 fTags[0].Tag:=tag;
 fTags[0].Qt:=HexStr(length(inttostr(Length(dados))),1);
 fTags[0].Tamanho:=Length(dados);
 fTags[0].Dados:=dados;
